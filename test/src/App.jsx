...
function AttractionList({
  ...
  function selectCity(value) {
    setFilterCity(value);
    if (value)
      onFetch({ skip: 0, city: value, type: filterType, search: filterSearch });
  ...
  function selectType(value) {
    setFilterType(value);
    if (value)
      onFetch({ skip: 0, city: filterCity, type: value, search: filterSearch });
    ...
    <div className="page">
          ...
          {attractions.map((a) => (
            ...
            >
              ...
              <p className="card-meta">
                ...
                {a.ticket_price != null && (
                  <span className="price">¥{a.ticket_price}</span>
                )}
              </p>
              {a.overview && (
      ...
      {attractionCount > limit && (
        <div className="pagination">
          <button
            disabled={skip === 0}
            onClick={() => goPage(Math.max(0, skip - limit))}
          >
            上一页
          </button>
          <span>
            第 {Math.floor(skip / limit) + 1} /{" "}
            {Math.ceil(attractionCount / limit)} 页
          </span>
          <button
...
function AttractionDetail({ attraction, onBack }) {
  ...
  const { transport_routes = [] } = attraction;
  ...
  return (
    <div className="page">
      ...
      <p className="card-meta">
        ...
        {attraction.type && <span className="tag">{attraction.type}</span>}
        {attraction.ticket_price != null && (
          <span className="price">¥{attraction.ticket_price}</span>
        )}
      </p>
      ...
      {attraction.overview && (
        <div style={{ marginBottom: "16px" }}>
          <h3>概述</h3>
          <p className="card-desc">{attraction.overview}</p>
        </div>
      )}
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <p className="card-info">
          {attraction.open_hours && (
            <span>开放时间: {attraction.open_hours}</span>
          )}
          {attraction.duration_of_visit && (
            <span>建议游览: {attraction.duration_of_visit}</span>
          )}
          {attraction.address && <span>地址: {attraction.address}</span>}
        </p>
      </div>

      {transport_routes.length > 0 && (
        <div>
          <h3>交通路线</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {transport_routes.slice(0, 5).map((route, idx) => (
              <div
                key={idx}
                style={{
                  padding: "12px",
              ...
              >
                ...
                >
                  <span>
                    {route.begin_time} - {route.end_time}
                  </span>
                  <span style={{ margin: "0 8px" }}>
                    历时 {route.duration} 小时
                  </span>
                </div>
              </div>
            ))}
...
export default function App() {
  ...
  async function viewDetail(attractionId) {
    try {
      const res = await fetch(`${API}/api/attractions/${attractionId}`);
      const data = await res.json();
      if (res.ok) {
  ...
  return (
    <div className="app">
      ...
      {msg && page !== "login" && page !== "register" && (
        <p className="msg-top" onClick={() => setMsg("")}>
          {msg}
        </p>
      )}
      {renderPage()}
